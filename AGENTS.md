...
- 项目决策与协作链路：`project-chain.ts` 管理版本化决策、通用交付物与状态转换；`project-chain-service.ts` 在项目 SQLite 的 `project_chain_revisions` 中追加快照，在同一事务内校验项目归属、任务责任和 `expectedRevision`。`ProjectCollaborationTasks` 复用权威任务、负责人、依赖及阻塞数据，不从名称推断关系。状态使用 Jotai。
  ...
  - **验证边界**：行为测试见 `project-chain.test.ts`、`project-chain-rules.test.ts`、`project-chain-service.test.ts` 和 `project-flow-metrics.test.ts`，身份目录回归见 `workflow-identity-service.test.ts`。`ProjectChainOverview` 只用显式 ID 展示决策链与协作链，`ProjectFlowHealth` 基于权威任务时间戳计算流动指标。结构化来源定位和低风险确定性自动验收已接入本地治理闭环；远端多人身份认证、外部证据自动对账、语义质量自动验收、交接消息发送、跨项目依赖与 Proactive 自动升级告警尚未接入。源码测试、构建和本地打包启动不等于生产多人协作与外部结果已经验收。
  - **DACI、ADR、DoD、依赖与执行关联**：关键 `decision` 带 DACI 时先以 `candidate` 保存，`driverId` 必须是主进程 actor，`approverId` 负责 `approve_decision`；所有 DACI 身份必须在本地目录启用，必须有未来 deadline，且影响任务归属当前项目。候选决策不能驱动交付；无 DACI 的旧/低风险决策兼容为 `decided`。关键决策的 `sourceRefs` 必须结构化保存 `sourceType`、`sourceId`、`locator` 和可选 `checksum`；旧字符串加载为 `legacy`，不能作为新关键决策拍板依据。决策修订用 `supersedes`／`supersededBy` 显式关联。`projectDefinitionOfDone` 与 `taskDefinitionOfDone` 在保存交付物时冻结为 `definitionOfDone`，人工验收必须用 `completedCriteria` 覆盖全部条目。`taskDodAutoAcceptance` 只允许本机管理员为低风险任务逐项配置 `artifact_reference_present` 或 `completed_execution`；提交时保存 `dodCheckResults`，全部通过才由 `system:dod-verifier` 追加 `auto_accept` 审计并转为 `accepted`，任一失败保持 `submitted`。这些验证器只证明引用或权威 Run 状态存在，不证明成果内容质量。`project-sqlite-store.updateTask()` 将任务置为 `completed` 前以 `assertTaskCompletionAllowed()` 再检查。`dependencyHandoffs` 只能绑定权威 `TaskDependency` 的上游／下游和负责人，经历 `planned → offered → accepted` 或 `returned`；接收时必须逐项确认标准，契约状态直接控制 `listTaskBlockers()` 的阻塞原因与解除。交付物的 `executionId` 必须精确指向同项目、同任务、已完成的 Agent execution，并冻结 `agentId`、`sessionId` 与 `completedAt`；Agent 负责人任务不允许省略。`serviceLevelDays` 配置 SLE，指标含 WIP、等待、30 天吞吐、平均周期、最老工作项与超时数。不要以成果引用、来源校验值、Agent Run 或本机记录宣称外部产物已真实验证。
