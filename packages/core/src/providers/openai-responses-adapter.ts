 ...
 * - 覆盖 o 系列与 GPT-5 系列模型的原生接口（非 Chat Completions 降级）
 *
 * 移植自 Proma 开源版（packages/core/src/providers/openai-responses-adapter.ts）。
 */
...
export class OpenAIResponsesAdapter implements ProviderAdapter {
  ...
  buildStreamRequest(input: StreamRequestInput): ProviderRequest {
    const url = resolveOpenAIResponsesUrl(input.baseUrl)
