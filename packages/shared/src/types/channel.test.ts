...
describe('Agent provider runtime capabilities', () => {
  ...
  test('Proma runtime 会把 Anthropic 兼容端点转换为 OpenAI-compatible 根路径', () => {
    ...
    expect(resolveAgentRuntimeBaseUrl('kimi-coding', 'proma', 'https://api.kimi.com/coding/v1/messages')).toBe('https://api.kimi.com/coding/v1')
    expect(resolveAgentRuntimeBaseUrl('kimi-coding', 'proma', 'https://api.kimi.com/coding/v1/chat/completions')).toBe('https://api.kimi.com/coding/v1')
