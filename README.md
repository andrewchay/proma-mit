...
- **项目决策与协作链路（本地规则层）**：项目详情的「决策与协作链路」展示任务负责人、上下游依赖、阻塞原因和 Agent 执行记录；通用交付物（代码、报告、设计成果等）关联任务与具体决策版本，并记录成果引用、验收标准及负责人／验收人／接收人的身份 ID。负责人必须匹配当前任务，责任人必须在本地身份目录中启用。交付由负责人提交、验收人验收；通过必须填写验收意见和依据。验收后由负责人发起交接，再由接收人确认或退回，不能跳过发起步骤。决策或交付修订必须说明变更原因；决策更新、任务改派或退回后须复核并保存新交付版本，旧验收不能沿用。版本和历史追加保存于项目 SQLite，并检查并发冲突。旧记录保留，缺少责任信息时不能继续流转，旧回执不会显示为新规则下的身份确认。当前操作身份固定为本机 `local-user`，不能代其他身份确认；允许同一身份承担多个职责，尚未强制职责分离。验收依据和成果引用仅记录、不自动验证真实性；尚未接入远端多人审批、交接消息发送、跨项目依赖与 Proactive 自动升级告警。现有页面已接入规则，尚未扩展新的链路图，也未完成本轮打包桌面交互验收。

- **关键决策、DoD 与依赖治理**：勾选为关键决策时，必须填写 DACI 的推进人、唯一拍板人、最迟决定时间、候选方案和影响任务；该决策先处于候选状态，只有拍板人确认后才可关联交付物。项目和任务可配置 Definition of Done；DoD 会冻结到交付物版本，验收人须逐项确认，任务状态写入 `completed` 时会再次检查。关键任务依赖可建立双方交接契约，绑定现有依赖边、上下游负责人、承诺时间和接收标准；下游在接收前不能完成。Agent 交付可关联一条精确的已完成执行记录 ID；若任务负责人是 Agent，该关联为必填。当前仍仅由本机 `local-user` 执行身份动作，远端身份认证、自动验收策略及可视化流动指标尚未接入。
