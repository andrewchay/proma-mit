...
const OUTPUT_OPTIONS = [
  { value: 'auto', label: '自动：Gravitas 激活时写入对话框，否则写入当前光标' },
  { value: 'clipboard', label: '仅复制到剪贴板' },
  { value: 'proma-input', label: '仅写入 Gravitas 输入框' },
]
...
export function VoiceInputSettings(): React.ReactElement {
  ...
  return (
    <div className="space-y-6">
      <SettingsSection
        title="豆包流式语音输入"
        description="通过全局快捷键唤起浮窗，实时识别语音，停止后写入 Gravitas 输入框或当前光标位置。"
        action={
          <Button
            variant="outline"
            size="sm"
      ...
      >
                ...
                <div>
                  ...
                  <span className="ml-2 text-muted-foreground">
                    {micPermission.status === 'granted'
                      ? '已授权，语音输入可正常使用'
                      : micPermission.status === 'denied'
                      ? '已被系统阻止，请在系统设置中允许 Gravitas 访问麦克风'
                      : micPermission.status === 'not-determined'
                      ? '未授权，使用语音输入前需要先授权'
        ...
        <SettingsCard>
          ...
          <SettingsSelect
            label="输出方式"
            description="默认写入当前光标位置；如果唤起时 Gravitas 是当前激活窗口，会写入当前 Chat 或 Agent 输入框。自动粘贴失败时会保留到剪贴板。"
