 ...
 *
 * 支持两种模式：
 * - Personal 备份（.gravitas-backup）：全量导出，含解密 API Key
 * - Share 分发（.gravitas-share）：自由选择组件，凭据自动剥离
 *
 * Share 模式支持多工作区导出：
...
export function MigrationSettings(): React.ReactElement {
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsSection
        title="导入备份"
        description="从备份文件导入数据，支持 .gravitas-backup 和 .gravitas-share 格式"
      >
        <button
