 ...
 *
 * 支持两种模式：
 * - Personal 备份（.gravi-backup）：全量导出，含解密 API Key
 * - Share 分发（.gravi-team）：自由选择组件，凭据自动剥离
 *
 * Share 模式支持多工作区导出：
...
type MigrationComponent = 'sessions' | 'skills' | 'mcp' | 'channels' | 'chattools' | 'evalsystems'
...
export function MigrationSettings(): React.ReactElement {
  ...
  const handleExport = async (): Promise<void> => {
    ...
    try {
      ...
      const components: MigrationComponent[] =
        exportMode === 'personal'
          ? ['sessions', 'skills', 'mcp', 'channels', 'chattools', 'evalsystems']
  ...
  return (
    <div className="space-y-8">
      ...
      <SettingsSection
        title="导入备份"
        description="从备份文件导入数据，支持 .gravi-backup 和 .gravi-team 格式"
      >
        <button
