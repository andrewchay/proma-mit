/**
 * AgentCommandSuggestion — `/` 统一命令菜单
 *
 * 输入 `/` 时打开统一命令菜单：
 * - root 页：调用 Skill / 引用会话 / 引用文件 / MCP 工具 / 添加附件 / 附加文件夹
...
import { createMentionPopup, isSuggestionTriggerPresent, positionPopup } from './mention-popup-utils'
import {
...
interface AgentCommandMenuProps {
  ...
  onInsertMention: (character: MentionCharacter, id: string, label: string) => void
...
function menuTitle(page: CommandPage): string {
  switch (page) {
    ...
    case 'tools': return 'MCP 工具'
    case 'sessions': return '引用会话'
    case 'files': return '引用文件'
    default: return '命令'
  }
}
...
function menuEmptyText(page: CommandPage, loading: boolean): string {
  ...
  switch (page) {
    case 'skills': return '没有已启用的 Skill'
    case 'tools': return '没有可用的 MCP 工具'
    case 'sessions': return '没有可引用的会话'
    case 'files': return '没有可引用的文件'
    default: return '没有匹配的命令'
  }
}

const AgentCommandMenu = React.forwardRef<AgentCommandMenuRef, AgentCommandMenuProps>(
  function AgentCommandMenu(
    ...
    const [fileResult, setFileResult] = React.useState<FileSearchResult>(EMPTY_FILE_RESULT)
    const [sessionSearchQuery, setSessionSearchQuery] = React.useState(query)
    const [pageEntryQuery, setPageEntryQuery] = React.useState('')
    const [debouncedFileSearchQuery, setDebouncedFileSearchQuery] = React.useState('')
    const pageQuery = page === 'root' ? query : getCommandMenuChildQuery(query, pageEntryQuery)
    const fileSearchQuery = page === 'files' ? pageQuery.trim() : ''
    const activeFileSearchQuery = page === 'files' && fileSearchQuery ? debouncedFileSearchQuery : ''
    const fileListRef = React.useRef<FileMentionRef>(null)
    const menuScrollRef = React.useRef<HTMLDivElement>(null)

    // 页面切换统一入口：重置选中索引（query 变化由页面内部 filter 反映，不重置）
    const changePage = React.useCallback((next: CommandPage): void => {
      setPage(next)
      setSelectedIndex(0)
    }, [])
    ...
    const selectReferenceItem = React.useCallback(
      (item: ReferenceMenuItem): void => {
        const character: MentionCharacter = page === 'skills' ? '/' : page === 'tools' ? '#' : '&'
    ...
    React.useImperativeHandle(ref, () => ({
      onKeyDown: ({ event }) => {
        ...
        if (event.key === 'ArrowUp') {
          setSelectedIndex((current) => getNextCommandMenuIndex(current, -1, items.length))
          return true
        }
    ...
    }), [page, rootItems, referenceItems, selectedIndex, selectRootItem, selectReferenceItem, changePage])

    const isFilePage = page === 'files'
    const hasItems = isFilePage
      ? fileResult.sessionEntries.length > 0 || fileResult.workspaceEntries.length > 0
    ...
    React.useEffect(() => {
      ...
      if (!container) return
      const selected = container.querySelector<HTMLElement>(`[data-command-menu-index="${selectedIndex}"]`)
      selected?.scrollIntoView({ block: 'nearest' })
    }, [isFilePage, selectedIndex])
    ...
    return (
      ...
      >
        <div className="flex items-center gap-1.5 border-b border-border/50 bg-muted/40 px-2.5 py-1.5">
          ...
          {page === 'skills' && (
            <span className="text-[10px] text-muted-foreground">继续输入以搜索</span>
          )}
          {loading && (
          ...
          {isFilePage ? (
            hasItems ? (
              <FileMentionList
                ...
                onSelect={(file: Pick<FileIndexEntry, 'name' | 'path'>) => onInsertMention('@', file.path, file.name)}
                onBack={() => changePage('root')}
                embedded
              />
            ) : (
            ...
            <div role="listbox" aria-label={menuTitle(page)}>
              {(page === 'root' ? rootItems : referenceItems).map((item, index) => {
...
export function createAgentCommandSuggestion(
  ...
  return {
    ...
    render: () => {
      ...
      const insertMention = (character: MentionCharacter, id: string, label: string): void => {
        activeProps?.command({
          id,
          label,
