...
import type { AgentSessionMeta, Channel } from '@gravitas/shared'
...
export function RoutinesTab(): React.ReactElement {
  ...
  const refresh = React.useCallback(async (): Promise<void> => {
    const [nextManifests, nextInstances, nextSessions, nextChannels] = await Promise.all([
  ...
  const targetChannel = channels.find((channel) => channel.id === session?.channelId && channel.enabled)
  React.useEffect(() => {
  ...
  const run = async (instance: RoutineInstanceView): Promise<void> => {
    if (!session || !modelId) { toast.error('请选择已配置渠道的会话及该渠道的模型'); return }
    setRunningId(instance.id)
    try {
      await window.electronAPI.proactive?.runRoutineInstance?.(instance.id, { sessionId: session.id, workspaceId: session.workspaceId, channelId: session.channelId, modelId, runtime: session.agentRuntime, prompt: '按 Routine 生成可审计结果；涉及长期记忆时仅输出 proma-memory-items 候选。', permissionMode: 'safe' })
