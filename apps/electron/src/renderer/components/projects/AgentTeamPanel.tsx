...
export function AgentTeamPanel(): React.ReactElement {
  ...
  const loadCapabilities = React.useCallback(async (agentId: string): Promise<void> => {
    try {
      const [versions, samples, observations, audits, health] = await Promise.all([
      ...
      setCapabilityData((current) => ({ ...current, [agentId]: { versions: versions as never[], samples: samples as never[], observations, audits, health } }))
    } catch (err) {
