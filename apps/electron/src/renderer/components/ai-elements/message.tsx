...
export const UserMessageContent = React.memo(
  function UserMessageContent({ children, className, ...props }: UserMessageContentProps): React.ReactElement {
    ...
    return (
      <div className={cn('relative inline-block max-w-full rounded-xl bg-muted/70 px-3.5 py-2.5', shouldCollapse && !isExpanded && 'pb-6', className)} {...props}>
        <div
          ref={contentRef}
          className={cn(
        ...
        {shouldCollapse && (
          <button
            ...
            className={cn(
              ...
              !isExpanded &&
                'absolute bottom-0 left-0 right-0 px-3.5 pb-2.5 pt-4 rounded-b-xl bg-gradient-to-t from-muted/70 to-transparent'
            )}
          >
            {isExpanded ? (
              <>
