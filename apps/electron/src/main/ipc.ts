...
import {
  ...
  getContentAudit,
  createContentAudit,
  updateContentAuditStatus,
  ensureCampaignWorkspace,
  listContentTracking,
  getContentTracking,
  createContentTracking,
  updateContentTrackingData,
  updateContentTrackingAnalysis,
  addPaidData,
...
import { listVideoAssets, deleteVideoAsset, renameVideoAsset } from './lib/video-asset-service'
import {
  ...
  isWorkflowComplete,
  getWorkflowProgress,
} from './lib/campaign-workflow-service'
...
export function registerIpcHandlers(): void {
  ...
  const assertMainTerminalRenderer = async (senderId: number): Promise<void> => {
    ...
    if (!mainWindow || mainWindow.webContents.id !== senderId) throw new Error('仅主窗口可以操作本地终端')
  }
  ipcMain.handle(TERMINAL_IPC_CHANNELS.CREATE, async (event, input) => {
    await assertMainTerminalRenderer(event.sender.id)
  ...
  ipcMain.handle(
    ...
    async (_, title?: string, channelId?: string, workspaceId?: string, agentRuntime?: AgentRuntime): Promise<AgentSessionMeta> => {
  ...
  ipcMain.handle(AGENT_IPC_CHANNELS.SET_PROACTIVE_SCHEDULE_ENABLED, async (_, scheduleId: string, enabled: boolean) => {
    ...
    return enabled ? resumeProactiveSchedule(scheduleId) : pauseProactiveSchedule(scheduleId)
  })
  ipcMain.handle(AGENT_IPC_CHANNELS.DELETE_PROACTIVE_SCHEDULE, async (_, scheduleId: string): Promise<void> => {
    if (typeof scheduleId !== 'string') throw new Error('定时任务 ID 无效')
    deleteProactiveSchedule(scheduleId)
  })
  ipcMain.handle(AGENT_IPC_CHANNELS.RUN_PROACTIVE_SCHEDULE, async (_, scheduleId: string) => {
    if (typeof scheduleId !== 'string') throw new Error('定时任务 ID 无效')
    return runProactiveScheduleNow(scheduleId)
  })
  ...
  // 自动拆分子任务后创建协作子会话（侧栏父子树面板）
  ipcMain.handle(
    AGENT_IPC_CHANNELS.SPLIT_AND_CREATE_COLLAB,
    async (_, input: import('@gravitas/shared').SplitAndCreateCollabInput): Promise<import('@gravitas/shared').CreateCollabDelegationsResult> => {
      return splitAndCreateCollabDelegations(input.parentSessionId, input.mainTask)
    }
  )
