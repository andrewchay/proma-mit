...
import type { ConversationMeta, FileAttachment, ChatMessage } from '@gravitas/shared'
...
export function createWelcomeConversation(): ConversationMeta | null {
  ...
  try {
    // 1. 创建对话
    const meta = createConversation('了解 Gravitas')

    // 2. 保存教程文件为附件
    const attachmentId = randomUUID()
    const attachmentFilename = 'Gravitas 使用教程.md'
    ...
    const userMessage: ChatMessage = {
      id: randomUUID(),
      role: 'user',
      content: '你好，我是 Gravitas 的新用户，希望快速上手。这是完整的使用教程，作为你的参考。',
      createdAt: now,
    ...
    const assistantMessage: ChatMessage = {
      id: randomUUID(),
      role: 'assistant',
      content: `你好，欢迎来到 Gravitas！Gravitas 是一个通用的 Agent，其实它可以完成任何事，说实话这也挺难的，因为你要构建完整的工作环境才能做到，这会涉及到一些新的概念或者思考方式，不过别担心，我们做了很多设计可以帮助你靠谱稳定的越用越好用。
...
了解你的背景之后，我会为你单独整理一份专属的 Gravitas 使用最佳实践——告诉你哪些功能最值得用、推荐的 Skills / MCP 配置，以及贴合你场景的工作流模板。
