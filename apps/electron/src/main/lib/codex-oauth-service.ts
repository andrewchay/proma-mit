 ...
 *
 * 复用 Pi SDK 内置的 Codex OAuth 流程完成登录：
 ...
 *
 * token 的加密存储与过期刷新由上层（channel-manager）负责，
 * 本服务只封装"跑一次登录流程""刷新一次 token"两个纯操作。
 *
 * 移植自 Proma 开源版（codex-oauth-service.ts），适配本项目
 * @earendil-works/pi-coding-agent 0.82.x 的 ModelRuntime API。
 */
...
/**
 * Pi 的 ModelRuntime 只要求 CredentialStore 的结构契约。OAuth 凭据仍由
 * channel-manager 加密持久化；这里使用内存 store，避免 Pi 写入自己的本地配置。
 */
...
export async function refreshCodexOAuth(refreshToken: string): Promise<CodexOAuthCredentials> {
  ...
  if (!refreshed) throw new Error('Codex OAuth 续签未返回凭据')
  return normalizeCredentials(refreshed)
}
