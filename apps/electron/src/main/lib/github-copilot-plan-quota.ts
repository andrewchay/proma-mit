/**
 * GitHub Copilot 订阅额度查询。
 *
 * 查询 copilot_internal/user 内部接口获取当前套餐的 premium/chat/completions
 * 剩余额度与重置时间。移植自 Proma 开源版（github-copilot-plan-quota.ts），
 * 适配本项目 proxy-fetch（getFetchFn，无连接池句柄）。
 */

import type { ChannelPlanQuotaResult, ChannelPlanQuotaWindow } from '@gravitas/shared'
import { parseGithubCopilotCredentials } from '@gravitas/shared'
...
/** 解析 copilot_internal/user 额度响应（纯函数，供测试）。 */
...
/** 查询当前订阅额度（需已登录的 Channel.apiKey 凭据 JSON）。 */
export async function queryGithubCopilotPlanQuota(
  ...
  try {
    const response = await getFetchFn(proxyUrl)(url, {
      method: 'GET',
      redirect: 'error',
