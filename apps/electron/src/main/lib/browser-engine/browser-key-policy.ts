/**
 * Browser Engine 按键语义与键码（纯逻辑，可单测）。
 *
 * 关键技术点：通过 CDP `Input.dispatchKeyEvent` 派发导航键时，必须携带
 * windowsVirtualKeyCode 才能被 Chromium 识别为真实按键并触发默认行为
 * （PageDown 滚动、Enter 提交表单、Tab 移动焦点等）。只传 key 字符串
 * 时 Chromium 无法为非字符导航键推断虚拟键码，默认行为不会发生。
 */

export interface BrowserNavigationKeyCode {
  /** DOM KeyboardEvent.code，通常与 key 一致 */
  code: string
 ...
 * 这让已聚焦的 input/textarea/contenteditable 可以一次输入完整消息，
...
export function parseBrowserPressAction(input: string): BrowserPressAction {
  ...
  // Input.insertText 支持换行；其他控制字符不应传给网页输入控件。
