/** 可复制的项目周报/摘要，数据全部来自项目模型、告警、Todo outbox 与活动流。 */
...
export async function generateProjectSummary(projectId: string): Promise<ProjectSummary> {
  ...
  const [progress, items, alerts, retries, activities] = await Promise.all([
  ...
  return { projectId, title: project.title, generatedAt, completed: progress.completed, total: progress.total, overdueCount: overdue.length, blockedCount: blocked.length, highRiskCount: highRisk.length, todoRetryCount: retries.length, recentActivityCount: activities.length, markdown: lines.join('\n') }
}
