...
describe('Proactive Scheduler newSession 模式', () => {
  ...
  test('本次完整输出持久化，失败重试追加新记录而不改写旧结果', async () => {
    const scheduler = createScheduler(() => 6_000_000)
    const schedule = scheduler.create({ title: '检查', sessionId: 's', channelId: 'c', runtime: 'proma', prompt: '检查', schedule: { type: 'interval', intervalMs: 60_000 } })
    scheduler.setRunner(async () => { throw new Error('模型已停用') })
    const failed = await scheduler.runNow(schedule.id)
    expect(failed.status).toBe('failed')
    scheduler.setRunner(async () => ({ output: '完整结果\n' + 'x'.repeat(800), outputSummary: '摘要' }))
    const retried = await scheduler.runNow(schedule.id)
    expect(retried.output).toHaveLength(805)
    expect(new ProactiveSchedulerStore().listRuns().find((run) => run.id === failed.id)?.error).toBe('模型已停用')
    expect(scheduler.listRuns()).toHaveLength(2)
    scheduler.dispose()
  })
  test('重启将遗留运行标记为中断，不永久显示运行中', async () => {
    const store = new ProactiveSchedulerStore()
    store.saveRun({ id: 'interrupted', sourceType: 'schedule', sourceId: 's', status: 'running', trigger: 'manual', startedAt: 1 })
    const scheduler = createScheduler(() => 6_000_000)
    await scheduler.recover()
    expect(store.listRuns()[0]).toMatchObject({ status: 'failed', endedAt: 6_000_000 })
    expect(store.listRuns()[0]?.error).toContain('中断')
    scheduler.dispose()
  })
