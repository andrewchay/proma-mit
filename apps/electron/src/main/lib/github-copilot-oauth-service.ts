 ...
 * 本服务只使用内存 CredentialStore，绝不写入本地 Pi 配置。
 *
 * 移植自 Proma 开源版（github-copilot-oauth-service.ts），适配本项目
...
/** 内存态 CredentialStore：登录/续签过程不落盘 */
...
export async function refreshGithubCopilotOAuth(
  ...
  const refreshed = await store.read('github-copilot')
  if (!refreshed) throw new Error('GitHub Copilot OAuth 续签未返回凭据')
