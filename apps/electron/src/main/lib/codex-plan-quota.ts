/**
 * ChatGPT (Codex) 订阅额度解析。
 *
 * 将 ChatGPT `GET /backend-api/wham/usage` 响应转换为统一的订阅 Plan 额度格式。
 * OpenAI 返回 primary / secondary 两个滚动窗口；通常分别对应 5 小时和每周，
 * 但按实际 duration 映射，以兼容未来的套餐窗口变更。
 *
 * 移植自 Proma 开源版（codex-plan-quota.ts）的纯函数部分；
 * 网络请求由 channel-manager 的 queryCodexPlanQuota 承担。
 */
...
/** 将 wham/usage 响应转换为统一额度格式（纯函数，供测试）。 */
