...
import { WECHAT_IPC_CHANNELS, WECHAT_ITEM_TYPE, WECHAT_MESSAGE_TYPE, WECHAT_MESSAGE_STATE } from '@gravitas/shared'
...
class WeChatBridge {
  ...
  private async handleMessage(msg: WeChatIncomingMessage): Promise<void> {
    ...
    if (!binding) {
      await this.client.sendText(chatId, '请先在 Gravitas 设置中选择 Agent 渠道。', contextToken)
      return
    }
