...
// ===== Provider-Agnostic Runtime 桥接 =====

/**
 * 为 Provider-Agnostic runtime（proma / ai-sdk）构建协作会话工具定义。
 * 复用同一份内部状态，用 RuntimeToolDefinition 格式暴露，供 AISDKRuntimeCore
 * 等通用工具循环调用。
 */
export function buildRuntimeCollaborationTools(ctx: CollaborationToolContext): Array<{
  name: string
  description: string
  parameters: Record<string, unknown>
  execute(input: Record<string, unknown>): Promise<string>
}> {
  const jsonText = (payload: unknown): string => JSON.stringify(payload, null, 2)
  ...
  const tools: Array<{
  ...
  }> = [
    {
      ...
      parameters: {},
      async execute() {
        return jsonText(getAvailableAgentModels(ctx))
      },
    },
    {
      name: 'mcp__collaboration__delegate_agent',
      ...
      parameters: {
        ...
        task: { type: 'string', description: '发送给子 Agent 的完整任务说明，必须自包含必要上下文' },
        expectedOutput: { type: 'string', description: '希望子 Agent 最终返回的格式或要点' },
        modelId: { type: 'string', description: '可选目标模型 ID' },
      },
      async execute(input) {
      ...
      parameters: {
        ...
        items: {
          ...
          items: {
            ...
            properties: {
              title: { type: 'string' },
              role: { type: 'string', enum: ['explore', 'research', 'implement', 'review', 'custom'] },
              task: { type: 'string' },
    ...
    },
    {
      ...
      parameters: {
        ...
        minCompleted: { type: 'number', description: 'mode=any 时至少等待完成的数量，默认 1' },
        timeoutSeconds: { type: 'number', description: '最长等待秒数，默认 1800' },
      },
      async execute(input) {
        const args = input as unknown as { delegationIds?: string[]; mode?: 'all' | 'any'; minCompleted?: number; timeoutSeconds?: number }
        const ids = args.delegationIds?.length
          ? args.delegationIds
          : Array.from(delegations.values())
            .filter((item) => item.parentSessionId === ctx.sessionId && item.status === 'running')
        ...
        const totalTargets = liveRecords.length + settled.length
        if (totalTargets === 0) {
          return jsonText({ delegations: [], note: '没有找到可等待的协作委派' })
        }
        const mode = args.mode ?? 'all'
        const minCompleted = args.minCompleted ?? 1
        const timeoutSeconds = Math.min(args.timeoutSeconds ?? DEFAULT_WAIT_SECONDS, MAX_WAIT_SECONDS)
    ...
    },
    {
      ...
      async execute(input) {
        const args = input as unknown as { includeCompleted?: boolean }
        const items = listKnownDelegations(ctx.sessionId)
        const delegationsResult = args.includeCompleted === false
          ? items.filter((item) => item.status === 'running')
          : items
    ...
    },
    {
      ...
      async execute(input) {
        const args = input as unknown as { delegationIds: string[] }
        return jsonText({
          delegations: args.delegationIds.map((delegationId) => getDelegationResult(ctx.sessionId, delegationId)),
        })
      },
    },
    {
      ...
      async execute(input) {
        const args = input as unknown as { delegationId: string }
        return jsonText(stopDelegation(ctx.sessionId, args.delegationId))
      },
    },
    {
      name: 'mcp__collaboration__stop_delegations',
    ...
    {
      name: 'mcp__collaboration__answer_delegation_question',
      description: '代答协作子会话的阻塞问题（AskUserQuestion）或审批权限请求（Permission）。从 delegation 的 pendingBlockedEvents 获取 blockedEventId。',
      parameters: {
        delegationId: { type: 'string', description: '子会话所属的委派 ID' },
        blockedEventId: { type: 'string', description: '要回答的阻塞事件 ID' },
        answers: { type: 'object', additionalProperties: { type: 'string' }, description: 'AskUserQuestion 的回答' },
        permissionBehavior: { type: 'string', enum: ['allow', 'deny'] },
      },
      async execute(input) {
        const args = input as unknown as { delegationId: string; blockedEventId: string; answers?: Record<string, string>; permissionBehavior?: 'allow' | 'deny' }
        const blocked = getBlockedEventById(args.blockedEventId)
        if (!blocked) throw new Error(`阻塞事件不存在: ${args.blockedEventId}`)
        if (blocked.resolved) return jsonText({ answered: false, note: '该阻塞事件已被解决' })
        ...
        if (blocked.type === 'ask_user' && blocked.askUserRequestId) {
          ...
          return jsonText({ answered: blocked.resolved, type: 'ask_user' })
        }
        ...
        if (blocked.type === 'permission' && blocked.permissionRequestId) {
          ...
          return jsonText({ answered: blocked.resolved, type: 'permission', behavior })
        }

        return jsonText({ answered: false, note: '无法匹配阻塞事件类型' })
      },
    },
    {
      name: 'mcp__collaboration__continue_delegation',
      ...
      async execute(input) {
        const args = input as unknown as { delegationId: string; message: string }
        const record = getDelegationRecordForContinuation(ctx, args.delegationId)
        if (!record) throw new Error(`未找到当前会话下的委派: ${args.delegationId}`)
        if (record.status === 'running') {
          throw new Error(`委派正在运行中，无法追加指令: ${args.delegationId}`)
