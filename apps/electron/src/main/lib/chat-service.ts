 ...
 * - 查找渠道、解密 API Key
 * - 管理 AbortController
 * - 调用 @gravitas/core 的 Provider 适配器系统
 * - 桥接 StreamEvent → webContents.send()
 * - 持久化消息到 JSONL + 更新索引
 * - 模块化工具的 function calling 循环（通过 ChatToolRegistry + ChatToolExecutor）
 *
 * 纯逻辑（消息转换、SSE 解析、请求构建）已抽象到 @gravitas/core/providers。
 */

import { randomUUID } from 'node:crypto'
import type { WebContents } from 'electron'
import { CHAT_IPC_CHANNELS } from '@gravitas/shared'
import type { ChatSendInput, ChatMessage, GenerateTitleInput, FileAttachment, ChatToolActivity, ProviderType } from '@gravitas/shared'
import {
  getAdapter,
  streamSSE,
  fetchTitle,
} from '@gravitas/core'
import type { ImageAttachmentData, ContinuationMessage } from '@gravitas/core'
