...
export class PostgresAgentRuntimeInteractionStore implements AgentRuntimeInteractionStore {
  ...
  async createInteraction(input: CreateAgentRuntimeInteractionInput): Promise<AgentRuntimeInteractionRecord> {
    ...
    await this.client.query(
      `INSERT INTO proma_runtime_interactions (tenant_id,user_id,request_id,session_id,task_id,kind,status,request_json,created_at,expires_at)
    ...
    return { ...input, requestId: input.request.requestId, sessionId: input.request.sessionId, status: 'pending', createdAt }
  }
  ...
  async resolveInteraction(scope: AgentRuntimeScope, requestId: string, response: AgentRuntimeInteractionResponse): Promise<AgentRuntimeInteractionRecord | undefined> {
    return this.updatePending(scope, requestId, 'resolved', response)
  }
  ...
  async expireInteractions(scope: AgentRuntimeScope, now: number): Promise<AgentRuntimeInteractionRecord[]> {
    const result = await this.client.query<Record<string, unknown>>(
      `UPDATE proma_runtime_interactions SET status = 'expired', resolved_at = $3
  ...
  private async updatePending(scope: AgentRuntimeScope, requestId: string, status: 'resolved' | 'cancelled', response?: AgentRuntimeInteractionResponse): Promise<AgentRuntimeInteractionRecord | undefined> {
    const result = await this.client.query<Record<string, unknown>>(
      `UPDATE proma_runtime_interactions SET status = $4, response_json = $5::jsonb, resolved_at = $6
       WHERE tenant_id = $1 AND user_id = $2 AND request_id = $3 AND status = 'pending' RETURNING *`,
      [scope.tenantId, scope.userId, requestId, status, response ? JSON.stringify(response) : null, Date.now()],
    )
    return result.rows[0] ? interactionFromRow(result.rows[0]) : undefined
  }
}
...
function interactionFromRow(row: Record<string, unknown>): AgentRuntimeInteractionRecord {
  return {
    ...
    taskId: stringOrUndefined(row.task_id), kind: row.kind === 'ask_user' ? 'ask_user' : 'permission',
