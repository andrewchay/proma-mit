...
> 更新于 2026-08-01（第九轮：Pi 思考模式 thinkingLevel 配置入口落地，矩阵待办清零）。
> 基于 `apps/electron/src/main/lib/` 源码梳理，参考 `~/LLM/Proma` 上游（v0.16.5）实现。
> 核心结论：**权限、Plan、AskUser、Goal、SubAgent、MCP、Collaboration、WebSearch/WebFetch、WebBridge/ComputerUse、代理、记忆、上下文压缩、Skills、流式期间追加输入、思考模式配置 四大 runtime 已全部对齐**。
> 剩余差异：无。
...
| **思考/推理模式** | ✅ **完整**：`AgentThinkingLevel` 类型 + `PiAgentQueryOptions.thinkingLevel` + 全局设置 `agentThinkingLevel`（settings）+ AgentView brain Popover「思考深度」下拉（off~max，Pi runtime 显示）+ orchestrator 传递 | ⚠️ 依赖 provider（DeepSeek 等有 reasoning） | ⚠️ 同 Proma | ✅ 可配 `ThinkingConfig` | proma-mit 的 Pi 走 openai-completions/anthropic-messages，thinkingLevel 直接传 Pi SDK，无需上游 openai-responses extension；**展示层思考块已默认收起**（ThinkingBlock 默认折叠） |
...
| **流式期间用户追加输入** | ✅ **已补齐**：`sendQueuedMessage` 走 `session.steer`（priority now）/`session.followUp`（排队）；interrupt 用 `session.abort` 软中断 + prompt 链重发（`pendingInterruptPrompts`）；Pi 原生 agent loop 在 agent_end 前 drain steer/followUp 队列 | ✅ **已补齐（自研）**：`sendQueuedMessage` 队列 + query 外层 while 循环（每轮结束检查队列，追加消息作为下一轮 userMessage 继续）+ `interruptQuery` 软中断（abort + 等队列续跑） | ✅ while 循环支持追加 | ✅ SDK 多 turn | 四 runtime 全部支持输出期间续说；**前端/IPC 入口已就绪**：`AGENT_IPC_CHANNELS.QUEUE_MESSAGE` + preload `queueAgentMessage` + AgentView 流式分支（interrupt: true 软中断续说） |
...
- [x] **Pi 流式期间追加输入**（2026-08-01 完成：`session.steer/followUp` + interrupt abort + prompt 链，`pi-agent-streaming-queue.test.ts`）
- [x] **流式追加前端/IPC 入口**（`AGENT_IPC_CHANNELS.QUEUE_MESSAGE` + preload `queueAgentMessage` + AgentView 流式分支，已随并行提交就绪）
- [x] **Pi 思考模式 thinkingLevel 配置入口**（2026-08-01 完成：settings `agentThinkingLevel` + orchestrator 传递 + AgentView brain Popover 思考深度下拉）