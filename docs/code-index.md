...
proma-mit/
├── apps/
│   └── electron/              # Electron 桌面应用（@proma/electron@0.9.49）
│       ├── src/
│       │   ├── main/          # 主进程 + 服务层
│       │   ├── preload/       # IPC 上下文桥接
...
│       └── scripts/           # 构建脚本
├── packages/
│   ├── shared/                # 共享类型、IPC 通道常量、工具函数（@proma/shared@0.1.20）
...
| `agent-runtime/` | ~500 | Provider-Agnostic Agent Runtime：工具循环、提示词构建、Read/Write/Edit/Bash/Grep 工具实现 |
...
| `adapters/provider-agnostic-agent-adapter.test.ts` | ~150 | Provider-Agnostic 适配器集成测试：模拟 DeepSeek Read → Edit 完整循环 |
