...
proma-mit/
├── apps/
│   └── electron/              # Electron 桌面应用（@proma/electron@0.9.51）
│       ├── src/
│       │   ├── main/          # 主进程 + 服务层
│       │   ├── preload/       # IPC 上下文桥接
...
│       └── scripts/           # 构建脚本
├── packages/
│   ├── shared/                # 共享类型、IPC 通道常量、工具函数（@proma/shared@0.1.22）
...
| `agent-runtime/` | ~600 | Provider-Agnostic Agent Runtime：工具循环、提示词构建、历史消息转换、Read/Write/Edit/Bash/Grep 工具实现；含权限检查、路径安全、命令注入防护 |
...
| `adapters/provider-agnostic-agent-adapter.ts` | ~330 | Provider-Agnostic Agent 适配器：基于 ProviderAdapter 的多轮工具调用循环 |
| `adapters/provider-agnostic-agent-adapter.test.ts` | ~220 | Provider-Agnostic 适配器集成测试：模拟 DeepSeek Read → Edit 完整循环、权限拒绝、请求体顺序、历史消息传入 |
