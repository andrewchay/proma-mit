...
| [x] | 检索日志补排序与分页位置（每库明细：sort/pageSize/offset/命中数/截断/错误） | `M7.2` |
...
| [x] | LLM 审查与规则 lint 区分展示（来源归属 + 模型建议强制降级） | `M7.3` |
| [x] | 修订回复 + 版本 diff（回复须指向稿件版本；主张引用增删可见） | `M7.3` |
| [x] | 导出包与 manifest（逐项验证等级 + 缺口 + 排除受限内容） | `M7.2` |
...
## 8. M7 领域验收与发布加固（约 1–2 人周）— 进行中

| 状态 | 任务 | 提交/证据 |
|---|---|---|
| [x] | 崩溃注入与恢复语义：半写尾行 vs 真实损坏的双信号判定 | `M7.1` |
| [x] | **性能缺陷修复**：事件追加 O(n²) → 真追加 + 索引缓存（28× 提速） | `M7.1` |
| [x] | 性能压测脚本：5,000 来源 / 50,000 证据 / 1,000 运行全部达成 | `scripts/academic-perf-smoke.ts` |
| [x] | 发布检查脚本（模块清单 / IPC 注册 / preload 桥 / 安全守卫 / 台账） | `scripts/academic-release-check.ts` |
| [x] | 七领域端到端验收脚本（**合成数据，非人工盲测**；7/7 通过，每域 16 步） | `scripts/academic-blind-acceptance.ts` |
| [ ] | 100 条跨领域标注样例的 precision/recall（**需人工标注**，Agent 不应自造标签） | 未开始 |
| [ ] | 打包产物启动 smoke | 未开始 |
| [ ] | 交付文档（README/storage-contract 更新需单独授权） | 未开始 |
...
- **2026-09-17 M7.3 批次**：审查来源区分、修订回复与 diff、七领域验收。审查发现明确区分 `rule-lint`（有规则名与实测值，可标 error）与 `llm-suggestion`（必须记录模型与理由，**severity 强制降级到 warning**，界面标注「未经复核」）——防止模型意见被当成确定性事实。稿件 diff 以章节为单位并**单独暴露主张引用增删**：正文一字未改但引用变了也标 modified，否则「换了依据」会被漏掉。修订回复约束「已处理」必须指向具体稿件版本（否则无从核对），草稿声明状态为作者声明、不代表审稿人认可。七领域验收脚本 7/7 通过（每域 16 步，含 6 处门禁拦截验证），脚本内明确标注「合成数据、非人工盲测、不验证结论正确性」。
- **2026-09-17 M7.2 批次**：检索日志补齐与可追溯导出包。检索运行现在记录**每库明细**（排序、pageSize、offset 分页位置、命中数、截断、错误）——失败库也留明细，便于复核「当时到底查了什么」，未指定排序时如实记为 `default` 而不猜测。导出包（manifest.json + report.md）刻意不做统一完成度分数，而是**逐项标注验证等级**（来源是否带外部标识、证据是否带定位器、主张是否经研究者确认、产物是否已算摘要）并**显式列出缺口**（截断检索、未校验产物、未确认主张、无标识来源）。manifest 声明未包含的受限内容类型（全文、身份映射、凭据、日志原文、restricted 材料），并写明「本导出包记录过程可追溯性与已知缺口，不是对结论正确性的证明」。
- **2026-09-17 M7.1 批次**：发布加固。**发现并修复一个真实性能缺陷**：`appendEvent` 旧实现每次「读全文件 → 改 → 重写全文件」，呈 O(n²)——实测 4,000 条追加需 20.1s，56,000 条压测无法完成。改为真追加写入 + 按文件 size/mtime 校验的内存索引缓存后，4,000 条降到 0.72s（28×），方案目标量级全部达成（5,000 来源 0.83s / 50,000 证据 40s / 全量重放 43ms）。缓存以 size+mtime 为失效条件，外部改动会触发全量重建，因此不会读到过期 revision 或漏掉 commandId。同时明确了崩溃恢复语义：**半写尾行需「末个非空行」且「文件无换行结尾」两个信号同时成立**才认定为中断（只有一个信号会误判）；追加时截掉未完成尾行而非补换行——补换行会把半写行变成完整坏行，反而更像真实损坏。
