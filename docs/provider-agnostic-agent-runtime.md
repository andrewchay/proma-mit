...
| 目标模式 | 当前状态 | 剩余工作 |
|---------|---------|---------|
| `allow-all` | 已有 `bypassPermissions`，效果等同 | 统一命名/映射，避免 UI 与 Runtime 语义不一致 |
| `ask` | `auto` 模式通过 `AgentPermissionService` 可弹审批 | 明确把 `ask` 映射为“所有非只读操作都弹审批” |
| `safe` | 未显式实现 | 增加更严格的本地 classifier：只允许白名单内只读工具与命令，默认拒绝写操作 |

#### 3.2 AskUser 工具
- `AgentPermissionService` 已能处理 `AskUserQuestion`，但 Provider-Agnostic Runtime 的工具注册表未注册该工具
- 需要：在 `tool-registry.ts` 增加 `AskUserQuestion` 工具定义与执行器，模型才能在 Runtime 路径主动提问
...
> **近期重点**：建议先做 3.1 权限模式映射 和 3.2 AskUser 工具，这两项是用户日常交互的阻塞项；子 Agent 可以排后。
