...
- [x] `sdkMessagesToChatMessages`：将持久化 SDKMessage 转换为 ChatMessage，保留最近 20 条，tool_use/tool_result 序列化为 XML 标签
...
| `adapters/provider-agnostic-agent-adapter.test.ts` | 5 | 完整工具循环 + 权限拒绝 + 无工具直接结束 + 历史消息传入 + streamSSE 重试；校验请求体顺序 |
...
| 包 | 本次变更前 | 本次变更后 |
|---|-----------|-----------|
| `@proma/shared` | `0.1.19` | `0.1.23` |
| `@proma/electron` | `0.9.48` | `0.9.52` |
