...
> 创建时间：2026-07-15  
> 状态：Phase 1 / Phase 2 / Phase 3 已完成，真实 DeepSeek API 验证通过；短期不考虑 Phase 4
...
├── mcp-client-cache.ts      # MCP 客户端跨会话缓存
├── mcp-client-secret-store.ts # MCP OAuth client_secret 加密存储
├── mcp-oauth-store.ts       # MCP OAuth token 加密存储
├── mcp-oauth-provider.ts    # MCP OAuth ClientProvider 实现
├── mcp-oauth-pending.ts     # MCP OAuth 授权 pending 注册表
├── tool-impls/
│   ├── read-tool.ts
│   ├── write-tool.ts
│   ├── edit-tool.ts
...
│   ├── ask-user-tool.ts
│   ├── agent-tool.ts        # Sub Agent 委派
│   └── mcp-resource-tools.ts # MCP 资源发现/读取
├── tool-impls/tool-impls.test.ts
└── mcp-oauth.test.ts        # MCP OAuth 单元测试
...
#### 3.3 子 Agent（Sub Agent，已完成）
- `tool-registry.ts` 已注册 `Agent` 工具（`agent-tool.ts`）
- `provider-agnostic-agent-adapter.ts` 在 `executeToolCalls` 中将 `Agent` 工具调用透传给 `runSubAgent` 回调
- `agent-orchestrator.ts` 新增 `runProviderAgnosticSubAgent`，为子代理创建独立 `ProviderAgnosticAgentAdapter` 实例：
  - 使用 `buildBuiltinAgents(false)` 获取内置子代理定义（`code-reviewer` / `explorer` / `researcher`）
  - 子代理 session ID 为 `sub-{parentSessionId}-{uuid}`，不污染主会话历史
  - 子代理默认使用 `bypassPermissions` 模式运行，避免向主会话 UI 发送未知 sessionId 的审批请求
  - 收集子代理所有 assistant 文本消息，拼接后作为工具结果返回主循环
- 支持参数：`agent_name`、`task`、`model`、`files`、`max_turns`

#### 3.4 MCP OAuth 支持（已完成）
- `@proma/shared` 扩展 `McpServerEntry`，新增 `McpServerAuthConfig`，支持 `none` / `bearer` / `oauthAuthorizationCode` / `oauthClientCredentials`
- 新增 `mcp-oauth-store.ts`：使用 `safeStorage` 加密存储 token 到工作区 `mcp-oauth-tokens.enc`
- 新增 `mcp-oauth-provider.ts`：实现 SDK 的 `OAuthClientProvider`
  - `client_credentials`：直接换取 access token
  - `authorization_code`：打开系统浏览器授权，通过 `proma://mcp-auth` DeepLink 回调完成授权码交换
- 新增 `mcp-oauth-pending.ts`：管理待完成的授权会话，按 `workspaceSlug:serverName` 索引 `transport.finishAuth`
- `McpClientManager` 为 http/sse transport 接入 `authProvider`，`bearer` 类型直接注入 `Authorization` 头
- `ProviderAgnosticAgentAdapter` 暴露 `onMcpAuthRequired` 回调；`AgentOrchestrator` 将其转换为 `mcp_auth_required` PromaEvent 推送到 UI
- `apps/electron/src/main/index.ts` 注册 `proma://` 协议，处理 macOS `open-url` 与 Windows `second-instance` 回调，完成 token 交换
- `McpServerForm` 增加认证方式选择与 OAuth 配置字段

#### 3.5 MCP 其他加固（已完成）

- **client_secret 加密存储**：新增 `mcp-client-secret-store.ts`，使用 `safeStorage` 对工作区 MCP 服务器的 OAuth `client_secret` 进行 OS 级加密，集中存放在 `~/.proma/mcp-client-secrets.json`；`mcp.json` 中只保留空占位，避免长期凭证明文落盘
- **MCP 资源工具**：新增 `ListMcpResourcesTool` / `ReadMcpResourceTool`，让 Agent 可以发现并读取 MCP 服务器暴露的 `resources`（只读，默认在 `safe` / `plan` 模式下放行）
- **跨会话连接缓存**：新增 `mcp-client-cache.ts`，按 `workspaceSlug` + 配置指纹复用已连接的 `McpClientManager`，最后一次使用后 5 分钟自动断开，减少重复 SSE/HTTP 握手与 OAuth token 读取
- **Renderer OAuth 提示 UI**：`McpOAuthBanner` 在 Agent 对话底部显示待授权的 MCP 服务器；DeepLink 回调完成后主进程通过 `agent:mcp-auth-resolved` 通知渲染进程自动清除；用户可手动关闭或终止 Agent
...
| 测试文件 | 用例数 | 说明 |
|---------|-------|------|
| `agent-runtime/tool-impls/tool-impls.test.ts` | 17 | Read / Write / Edit / Bash / Grep / Agent 单元测试；含 symlink 绕过、Grep shell 注入、Sub Agent 委派 |
...
| `adapters/provider-agnostic-agent-adapter.test.ts` | 10 | 完整工具循环 + 权限拒绝 + 无工具直接结束 + 附件传入 + 历史消息传入 + plan 模式拒绝 + AskUserQuestion + safe 模式 + streamSSE 重试 + Agent 工具回调；校验请求体顺序 |
| `agent-runtime/mcp-oauth.test.ts` | 10 | token 加密存储、OAuth provider 元数据、client_credentials token 请求参数、pending 注册表 |
...
| `agent-runtime/mcp-client-secret-store.test.ts` | 4 | client_secret 保存/读取/删除/更新 |
| `agent-runtime/mcp-client-cache.ts` | - | 由适配器集成路径隐式覆盖 |
| `agent-runtime/prompt-builder.test.ts` | 4 | system prompt 构建、SDKMessage → ChatMessage 转换、历史截断 |
| `agent-runtime/retry.test.ts` | 5 | withRetry 成功、重试、不可重试、最大次数、中止信号 |
